...
> **状態（2026-08-19時点）**：Step 1〜11のコードはすべて実装済みで、自動テスト（`tests/smartLane.test.html` 全20件、`tests/appLogic.test.html` 全件）は通過しています。ただし下記「完了チェック」のうち実車走行が前提の項目は、まだ実車で確認していません。Phase 2（全画面UI・設定画面・音声検索）とPhase 2A（実走用ナビパネル・到着判定・標準音声案内など、`docs/04_COCCHi比較・今後の改善計画.md`参照）はこの計画の後に実装済みです。
...
- [ ] iPhone Safariで目的地設定→ナビ開始→到着まで動く（実車確認：未実施）
- [ ] 画面が消えず、音声が鳴る（実車確認：未実施）
- [x] 車線データの有無・GPS精度が画面で判別できる（DEBUGパネルに実装済み。実車確認：未実施）
- [x] 走行ログがJSONで取れる（道路種別・testRouteId・比較結果を含む）（実装・書き出し機能とも確認済み）
- [x] SmartLane が全20テストケースを通過する（`tests/smartLane.test.html` 20/20 パス）
- [x] 集計画面で全体／道路種別／ルート／一致状況が確認できる（`stats.html` 実装済み。実データでの確認は実車走行後）
- [ ] 上記4ルートの実走完了（未実施）
...
- **VOICEVOXによるナビ音声**：詳細仕様は `docs/03_VOICEVOXナビ音声仕様（将来フェーズ）.md` を参照。着手前に必ず「Webアプリのまま実現できるか、ネイティブ化が必要か」を調査すること
